...
func StartStream(under types.Storager) {
	...
	for _, method := range []string{stream.PersistMethodAppend, stream.PersistMethodMultipart, stream.PersistMethodWrite} {
...
func newStream(persisMethod string, under types.Storager) (*stream.Stream, error) {
	upper, err := NewStoragerFromString(fmt.Sprintf("%s/%s", upperStorageConnString, stream.PersistMethodWrite))
	if err != nil {
		return nil, err
	}
